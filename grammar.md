...
block = "{" { statement } "}" ;
statement = variable | assignment | if_block | while | for | expression | control_flow ;
...
for_conditions = ( ( variable | assignment ) ";" expression ";" expression ) | ( variable [ "," variable ] "in" expression ) ; 
...
char_literal = "'" .+ "'" ;
string_literal = '"' .+ '"';
number_literal = [ sign ] ( hex | float | int ) ;
sign = "+" | "-" ;
hex = "0x" ( "0" ... "9" | "a" ... "f" | "A" ... "F" )+ ;
float = [ int ] "." int+ ;
int = ("0" ... "9" )+ ;
