(module $the
    ...
    (func $__str_replace_String_String (export "__str_replace_String_String") (param $old i32) (param $new i32) (param $str i32) (result i32)
        (local $replaceStr i32)
        (local $len i32)
        (local $newLen i32)
        (local $oldLen i32)
        (local $newStart i32)
        ...
        (local $match i32)
        (local $start i32)
        (local $replaceStart i32)
        (local $char i32)
        (local $i i32)
        (local $j i32)
        (local $k i32)
        (local $last_insert i32)
        (local.set $len (call $__str_length (local.get $str)))
        (local.set $newLen (call $__str_length (local.get $new)))
        (local.set $oldLen (call $__str_length (local.get $old)))
        (local.set $newStart (i32.add (local.get $new) (i32.const 4)))
        (local.set $oldStart (i32.add (local.get $old) (i32.const 4)))
        (local.set $start (i32.add (local.get $str) (i32.const 4)))
        (if (i32.or 
            (i32.eq (local.get $old) (local.get $new)) 
            (i32.gt_s (local.get $oldLen) (local.get $len))
            )
            (then
                (call $__str_slice (local.get $str) (i32.const 0) (local.get $len)) ;; make a copy
                (return)
        ...
        (if (i32.eqz (local.get $len))
            (then 
                (if (i32.eqz (local.get $newLen))
                    (then
                        (local.set $replaceStr (call $__malloc (i32.const 4)))
                        (i32.store (local.get $replaceStr) (i32.const 0))
                        (return (local.get $replaceStr))
                    )
                    (else
                        (if (i32.eqz (local.get $oldLen))
                            (then
                                (local.set $replaceStr (call $__malloc (i32.add (local.get $newLen) (i32.const 4))))
                                (i32.store (local.get $replaceStr) (local.get $newLen))
                                (memory.copy
                                    (i32.add (local.get $replaceStr) (i32.const 4))
        ...
        (if (i32.eqz (local.get $oldLen))
            (then
                (call $__str_concat (local.get $new) (local.get $str))
                (return)
            )
        )
        (local.set $replaceLen 
            (i32.add
                (i32.sub (local.get $len) (local.get $oldLen))
                (local.get $newLen)
            )
        )
        (call $__malloc (i32.add (local.get $replaceLen) (i32.const 4)))
        (local.set $replaceStr)
        (i32.store (local.get $replaceStr) (local.get $replaceLen))
        (local.set $replaceStart (i32.add (local.get $replaceStr) (i32.const 4)))
        (loop $replace_loop (block $exit_replace_loop
            (i32.eq (local.get $i) (local.get $len))
            (br_if $exit_replace_loop)
            (i32.load8_u (i32.add (local.get $start) (local.get $i)))
            (local.set $char)
            (if (i32.and 
                (i32.eq (local.get $char) (i32.load8_u (local.get $oldStart)))
                (i32.eqz (local.get $replaced))
                )
                (then
                    (local.set $match (i32.const 1))
                    (local.set $k (local.get $i))
                    (local.set $j (i32.const 0))
                    (loop $check_match_loop (block $exit_check_match_loop
                        (i32.eq (local.get $j) (local.get $oldLen))
                        (br_if $exit_check_match_loop)
                        (if (i32.eq 
                            (i32.load8_u (i32.add (local.get $start) (local.get $k)))
                            (i32.load8_u (i32.add (local.get $oldStart) (local.get $j)))
                        )
                            (then
                            ...
                            (else
                                ...
                                (br $exit_check_match_loop)
                            )
                        )
                        (local.set $j (i32.add (local.get $j) (i32.const 1)))
                        (br $check_match_loop)
                    ))
                    (if (i32.eq (local.get $match) (i32.const 1))
                        (then
                            (local.set $i (i32.add (local.get $i) (local.get $oldLen)))
                            (memory.copy
                                (i32.add (local.get $replaceStart) (local.get $last_insert))
                                (local.get $newStart)
                                (local.get $newLen)
                            )
                            (local.set $last_insert (i32.add (local.get $newLen) (local.get $oldLen)))
                            (local.set $replaced (i32.const 1))
                            (br $replace_loop)
                        )
                        (else
                            (i32.store8 (i32.add (local.get $replaceStart) (local.get $last_insert)) (local.get $char))
                        )
                    )
                )
                (else
            ...
            (local.set $last_insert (i32.add (local.get $last_insert) (i32.const 1)))
            (local.set $i (i32.add (local.get $i) (i32.const 1)))
            (br $replace_loop)
        ))
    ...
    (func $__str_replaceAll_String_String (export "__str_replaceAll_String_String") (param $old i32) (param $new i32) (param $str i32) (result i32)
        (local $replaceStr i32)
        (local $len i32)
        (local $newLen i32)
        (local $oldLen i32)
        ...
        (loop $replace_loop (block $exit_replace_loop
            ...
            (if (i32.eqz (local.get $oldLen))
                (then
                    ...
                    (local.set $last_insert (i32.add (local.get $last_insert) (local.get $newLen)))
                    (i32.store8 (i32.add (local.get $replaceStart) (local.get $last_insert)) (local.get $char))
                )
                (else
                    (if (i32.eq (local.get $char) (i32.load8_u (local.get $oldStart)))
                        (then
                            (local.set $match (i32.const 1))
                            (local.set $k (local.get $i))
                            (local.set $j (i32.const 0))
        ...
        (if (i32.eqz (local.get $oldLen))
            (then
                (return (call $__str_concat (local.get $replaceStr) (local.get $new)))
            )
        )
        (local.get $replaceStr)
    )
    ...
    (data $__str_const4 (i32.const 350) "\02\00\00\00ll")
    ;;(data $__str_const1 (i32.const 200) "\02\00\00\00!\n")
    (func $main (export "main") (result i32)
        (call $println (i32.const 100))
        ;;(call $__str_replace_String_String (i32.const 350) (i32.const 500) (i32.const 100))
        (call $__str_replace_String_String (i32.const 350) (i32.const 350) (i32.const 100))
        (call $__str_replaceAll_String_String (i32.const 310) (i32.const 350) (i32.const 100))
        (call $println)
        (i32.const 0)
