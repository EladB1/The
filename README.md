...
The semantic analysis will do several things:
 - type/scope checking
 - access/mutability checking
 - scope tree and symbol table building
...
In order to make the next phases of compilation easier, the semantic analyzer will update the AST to provide more context to certain nodes

The main type of annotation is to label the node's type

For something like a function overload (multiple functions with the same name and return type, but different parameters), the semantic analyzer will tag the function definition and calls with the IRName. This can save the IR generator from having to look through the overloads since the semantic analyzer already does that.

Reasons for doing this:

- Eliminate repeated work
- Reduce IR/code generation complexity
- Save scope tree/symbol table lookups
- Provide evidence of successful semantic analysis

### IR Generation

The Intermediate Representation (IR) generated by the compiler is a custom Three Address Code (TAC) IR that lives entirely in memory. The IR generator takes in the AST and scope tree in order to lower the program into a form that is closer to WAT.

#### Temporary Variables
Temporary variables are used for any operation which produces some form of data. For example this source code:
